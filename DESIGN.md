...
- **Page `<h1>`** — one tier above "big h2", reserved for a page's own `<h1>` (the design goes up to 90px, bigger than any in-page section heading): `font-serif text-5xl font-medium md:text-8xl`, same no-`tracking-*` rule. Used in `TripsPage`, `ContactsPage`, `TripHero` — including the trip hero, even though its design mockup went bigger (up to 120px): kept it to one page-`<h1>` tier sitewide instead of introducing a second, larger one just for that page.
- **TripCard** (`src/components/TripCard.tsx`) — a reusable trip photo card (used in `UpcomingTrips` on the home page and in the `/trips` grid): `aspect-[3/4]`, `rounded-lg`, a darkening gradient at the bottom for legibility — the same `color-mix(in oklch, var(--color-charcoal) N%, transparent)` technique as the gradient in `Hero`, but one-sided (bottom to top, no darkening at the top), a region `Badge`, a heading `text-3xl md:text-4xl font-serif font-semibold` (30→36px, closer to the design's 34→46px than the "trip title" tier from Hero) — smaller than both the section's "big h2" and the active trip title in Hero, dates/price use the same `font-mono text-sm font-bold tracking-widest uppercase` pairing as the dates in Hero. Hover: the photo is muted by default (`saturate-50`), "comes alive" with color on hover (`saturate-100`) and zooms in slightly (`scale-110`) — plain CSS (`group`/`group-hover`, `transition-all duration-700`), no JS.
...
- **TripHero** (`src/app/(pages)/trips/[slug]/_components/TripHero.tsx`, page-only) — the cinematic full-photo hero for a trip's own page: `min-h-[calc(100dvh-(--spacing(17)))]` — Tailwind v4's `--spacing(N)` function shorthand for "N steps of the theme's base spacing unit" (`0.25rem`, `node_modules/tailwindcss/theme.css`), i.e. exactly what `h-17` itself computes to; referencing it directly keeps this tied to `Header`'s `h-17` step instead of a hardcoded `4.25rem` that just happens to match today; `min-h-`, not a fixed `h-`, so a trip with a longer title/more content grows the section instead of clipping against `overflow-hidden`), content pinned to the bottom (`flex flex-col justify-end`), a bottom-only darkening gradient (same `color-mix` technique as `Hero`/`TripCard`), a `Container fullWidth` (unlike `TripsPage`/`ContactsPage` — the design's hero content isn't capped at `max-w-7xl`, only padded), two `Badge`s (dates + trip format, `terracotta`/`charcoal` variants), the "Page `<h1>`" tier (see above) capped to `w-1/2` of the container with `text-balance` so a long title wraps onto a couple of even lines instead of running the full width, then a stat row (days/km/price, `font-mono text-2xl font-bold` values over `text-base tracking-widest uppercase text-cream-400` labels) plus a "Присоединиться" button reusing `Cta`'s exact button recipe, currently linking to `/contacts` (no application-form section yet). `TripPage` (`/trips/[slug]`) is meant to grow more sections over time, and not every trip is expected to use every section — that's still an open question, not decided yet.

## Content: preview vs. detail

`src/content/trips.ts` (`Trip`) and `src/content/tripDetails.ts` (`TripDetail`) are deliberately separate, even though right now they're both static arrays with overlapping placeholder data. `Trip` is the lightweight shape used for listing UI (`TripCard`, `UpcomingTrips`, `TripsPage`'s grid) — id, slug, title, region, dates, price, photo. `TripDetail` is the full shape a trip's own page needs (`TripHero` today, more later) — currently adds `format`/`days`/`distance` on top of most of the same fields, looked up by `slug` alone (no `id`/`region` yet, since nothing on the detail page needs them). This split exists because the two will eventually come from two different API calls (a list endpoint vs. a per-slug detail endpoint) — the listing page was never meant to fetch full detail data for every trip just to render a card.
