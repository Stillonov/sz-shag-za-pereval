...
`TripDetail` itself is grouped by page section (`hero`, `about`, more as the page grows) rather than kept flat — each page-only section component (`Hero`, `About`) reads only its own nested object off `trip`, so a section's fields stay scoped to the component that uses them instead of one flat bag everyone reads from. `slug`/`title` stay top-level since they identify the trip as a whole rather than belonging to one section.
