...
- **TripHero** (`src/app/(pages)/trips/[slug]/_components/TripHero.tsx`, page-only) — the cinematic full-photo hero for a trip's own page: `min-h-[calc(100dvh-(--spacing(17)))]` — Tailwind v4's `--spacing(N)` function shorthand for "N steps of the theme's base spacing unit" (`0.25rem`, `node_modules/tailwindcss/theme.css`), i.e. exactly what `h-17` itself computes to; referencing it directly keeps this tied to `Header`'s `h-17` step instead of a hardcoded `4.25rem` that just happens to match today; `min-h-`, not a fixed `h-`, so a trip with a longer title/more content grows the section instead of clipping against `overflow-hidden`), content pinned to the bottom (`flex flex-col justify-end`), a bottom-only darkening gradient (same `color-mix` technique as `Hero`/`TripCard`), a `Container fullWidth` (unlike `TripsPage`/`ContactsPage` — the design's hero content isn't capped at `max-w-7xl`, only padded), two mono pill badges (dates + trip format, `bg-terracotta`/`bg-charcoal`), the "Page `<h1>`" tier (see above) capped to `w-1/2` of the container with `text-balance` so a long title wraps onto a couple of even lines instead of running the full width, then a stat row (days/km/price, `font-mono text-2xl font-bold` values over `text-base tracking-widest uppercase text-cream-400` labels) plus a "Присоединиться" button reusing `Cta`'s exact button recipe, currently linking to `/contacts` (no application-form section yet). `TripPage` (`/trips/[slug]`) is meant to grow more sections over time, and not every trip is expected to use every section — that's still an open question, not decided yet.
